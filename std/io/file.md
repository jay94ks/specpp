...
- **`platform: web`(SPEC.md 3.10절)에서는 이 패키지를 그대로 쓸 수 없다** —
  브라우저 샌드박스에는 임의 경로를 읽고 쓸 수 있는 파일시스템이 없다.
  대상이 웹이면 이 패키지 대신 [`std/web/storage.md`](../web/storage.md)를
  쓴다 — 다만 그쪽은 경로가 아니라 키(key) 하나로 값을 저장하는, 모양이
  다른 계약이라는 점에 주의한다(단순 재바인딩이 아니다).
...
- 파일 잠금은 이 버전의 범위 밖이다.
- 전체를 메모리에 올리지 않고 부분적으로 읽고 쓰는 것은 이 파일
  자체는 다루지 않는다 — [`std/io/stream.md`](stream.md)를 쓴다.
