...
(`kind: native`, SPEC.md 1.2절). [`std/cloud/s3.md`](s3.md)로 올린 객체를
...
정적 자산을 바꾼 뒤([`std/cloud/s3.md`](s3.md)의 `putObject`로),
...
- 캐시 유효 기간(TTL)은 CDN 쪽 설정이 아니라 **오리진 객체의
  `Cache-Control`/`Expires` 헤더**로 정한다 — [`std/cloud/s3.md`](s3.md)의
