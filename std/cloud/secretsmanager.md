...
코드에 심지 않는다([`std/cloud/s3.md`](s3.md) Constraints 등) — 를
...
- **자동 회전(rotation)**을 설정하면 정해진 주기마다 값이 자동으로
  바뀐다 — 실제로는 회전용 [`std/cloud/lambda.md`](lambda.md) 함수가
...
- 시크릿에 대한 세밀한 접근 제어(리소스 정책)는
  [`std/cloud/iam.md`](iam.md)의 범위이며 여기서는 다루지 않는다.
